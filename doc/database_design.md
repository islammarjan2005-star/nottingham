...
copies of each student's details. That would
...
stored once, and the  SQL becomes a set of simple
JOINs rather than string matching on long labels.

I chose SQLite because it is file-based, needs
...
- **courses** - course names in the source data are long strings
  ...
  keeps the `students` table narrow and avoids typo
  duplicates.
...
- **outcomes** - the spreadsheet uses Quality Manual codes A to H
  ...
  Other) means every  query can JOIN on the code and
  pick up a clean category without having to repeat a `CASE WHEN` in each query.
  
- **claims** - the main (fact) table, one row per (form, module)
  pair. PostID is not
  unique as a single form can list several affected modules so I
  added incrementing `claim_row_id` as the primary key and
...
- **claim_updates** - the admin columns (Campus updated,
...
- **Star schema with a separate date dimension.** Overkill for
  ~1,200 rows and four  questions;  date strings give
...
- Identifiers (`student_id`, `module_code`, `claim_id`) are stored
  ...
  characters (`FRM00712`, `COMP4031`). Casting them to `INTEGER` would lose that format.
  
- Dates are stored as (`YYYY-MM-DD`). SQLite has no
  dedicated date type, but these strings sort lexicographically in
  date order and work directly with `julianday()` and `strftime()`,
  which is everything Q1-Q4 need for date arithmetic and month
  grouping.
  
- `outcomes.category` is a 'denormalisation'.
  It could be derived from `outcome_code` in every query via a
  `CASE WHEN`, but storing it once on the outcomes row keeps the
   SQL short and removes the risk of the  logic
